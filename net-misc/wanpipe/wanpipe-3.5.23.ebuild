...
src_prepare() {
	...
	#
	epatch "${FILESDIR}/${PN}-3.5.22-Makefile.patch"
	...
	if ! use vanilla ; then
		...
		epatch "${FILESDIR}/${PN}-3.5.23-QA-fix-parallel-make.patch"
	...
	# fix clash between wancfg_legacy and top-level Makefile install targets
	# ("/var/.../image//etc/wanpipe/lib/: file already exists")
	epatch "${FILESDIR}/${PN}-3.5.22-wancfg_legacy-Makefile.patch"
