...
IUSE_MODULES="
	abstraction alsa amr amrwb avmd blacklist bv callcenter +cdr_csv cdr_mongodb cdr_pg_csv cdr_sqlite celt
	cepstral cidlookup cluechoo codec2 com_g729 commands conference +console curl dahdi_codec +db dialplan_asterisk
	dialplan_directory dialplan_xml dingaling directory distributor dptools easyroute enum erlang_event esf esl
	event_multicast event_socket event_test event_zmq expr fax fifo flite freetdm fsk fsv +g723_1 g729 gsmopen h26x h323
	+hash httapi http_cache +ilbc isac java json_cdr khomp ladspa lcr ldap +limit +local_stream +logfile loopback +lua
	managed memcache mongo mp4 mp4v native_file nibblebill opal opus osp perl pocketsphinx portaudio portaudio_stream
	posix_timer python rad_auth radius_cdr random redis reference rss rtmp sangoma_codec shell_stream shout silk siren
	skel skel_codec skinny skypopen sms snapshot +sndfile snipe_hunt snmp snom +sofia soundtouch +spandsp +speex spidermonkey
	spy stress +syslog theora timerfd +tone_stream tts_commandline unicall unimrcp valet_parking vlc vmd +voicemail voicemail_ivr
	voipcodecs xml_cdr xml_curl xml_ldap xml_rpc xml_scgi
"

## temporary broken:
# yaml

IUSE_MODULES_EXTERNAL="squirrelfish ssh"

# mod_say_X
IUSE_LINGUAS="de +en es fa fr he hr hu it ja nl pt ru th zh"
...
for x in ${IUSE_MODULES_EXTERNAL}; do
	IUSE="${IUSE} ${x//[^+]/}freeswitch_modules_${x/+}"
done
...
pkg_setup() {
	...
	enewuser freeswitch -1 -1 /var/lib/run/freeswitch freeswitch
...
esl_doluamod() {
        ( # dont want to pollute calling env
                insinto /usr/$(get_libdir)/lua/$(${LUA:-/usr/bin/lua} -e "print(_VERSION)" | sed -e '1s:^Lua \([0-9]\.[0-9]\):\1:')
		insopts -m755
                doins "$@"
        ) || die "failed to install $@"
}
